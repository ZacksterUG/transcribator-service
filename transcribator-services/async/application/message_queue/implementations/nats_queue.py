...
class NatsQueue(IMessageQueue):
    ...
    async def subscribe(self, topic: str, handler: Callable) -> None:
        ...
        if self.use_jetstream:
            ...
            # Создаём durable consumer name
            durable_name = f"{topic.replace('.', '_')}_consumer"
            ...
            config = ConsumerConfig(
                ack_wait=300,      # 5 минут на обработку (в секундах)
                max_deliver=5,     # максимум 5 попыток доставки
                deliver_policy="all"  # или "last", "new" в зависимости от нужд
            )
            
            # Подписываемся с конфигурацией
